...
public class PendulumCenter extends JPanel
{
    ...
    ArrayList<Wave_Pendulum> singlePen = new ArrayList<Wave_Pendulum>();
    ...
    public PendulumCenter()
    {
        ...
        for(int i=0;i<200;i++)
            {
                singlePoint.add(new Wave_Pendulum(.3, 400, 400, (Math.PI/2)+offset*i,0).calculate(timestep));
                singlePen.add(new Wave_Pendulum(.3, 400, 400, (Math.PI/2)+offset*i, 0));
            }
        for(int i=0;i<200;i++)
            {
